...
class TeamsFileUploadBot extends TeamsActivityHandler {
    constructor() {
        ...
        this.onMessage(async (context, next) => {
            ...
            if (attachments && attachments[0] && attachments[0].contentType === 'application/vnd.microsoft.teams.file.download.info') {
                ...
                const base64EncodedData = Buffer.from(fs.readFileSync(filePath)).toString('base64');
                let returnFromAOAI = await this.callGPT4Vision(base64EncodedData);
                const reply = MessageFactory.text(`<b>${returnFromAOAI}</b>`);
    ...
    //     await context.sendActivity('Channel created');
    //     await next();
    // }
    ...
    async callGPT4Vision(base64EncodedData) {
        const client = new OpenAIClient(
            "",
            new AzureKeyCredential("")
        );
        const deploymentId = "gpt-4-vision";
        const prompts = [
            { role: "system", content: "You are a helpful assistant. You will talk like 10 years old boy." },
            {
                role: "user", content: [
                    {
                        "type": "text",
        ...
        const result = await client.getChatCompletions(deploymentId, prompts, { maxTokens: 2000 });
        const resultText = result.choices[0].message.content;
        console.log(`Chatbot: ${resultText}`);
